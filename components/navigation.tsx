...
export default function Navigation({ activePage }: NavigationProps) {
  ...
  const navTabs = [
    { href: "/retail-dex", label: "Dex Trading", id: "retail-dex" as const },
    { href: "/base-app", label: "Base App", id: "base-app" as const },
    { href: "/developer-platform", label: "Dev Platform", id: "developer-platform" as const },
  ]
  ...
  return (
    ...
    >
      ...
      {isMenuOpen && (
        <div 
          className="absolute left-0 right-0 top-full z-50 border-b bg-white shadow-lg animate-slide-down" 
            ...
            {navTabs.map((tab, index) => (
              <Link
                ...
                className={`border-b py-4 text-base font-medium transition-colors duration-200 hover:text-[var(--text-secondary)] ${
                  activePage === tab.id ? 'text-[var(--text-secondary)]' : 'text-[var(--text-muted)]'
                }`}
                style={{ 
